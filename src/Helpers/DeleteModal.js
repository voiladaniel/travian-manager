import React from 'react'
import { Modal, Button, Form, Col } from 'react-bootstrap'

export const DeleteModal = ({ show, handleClose, handleChange, submitHandler, data, isLoading, isLoadingSettings }) => {
  return (
    show ?
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Delete</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <Form onSubmit={submitHandler}>
              <Form.Row>
                <Form.Group as={Col}>
                  <Form.Label>Are you sure you want to delete plan: {data.name} ?</Form.Label>
                </Form.Group>
              </Form.Row>
              <hr />
              <Form.Row className="justifyCenter">
                <Button variant="secondary" onClick={handleClose}>
                  Close
                </Button>
                &nbsp;
                <Button variant="primary" type="submit" disabled={isLoading}>
                  Delete
                </Button>
              </Form.Row>
              {isLoading &&
                  <>
                    <span>Loading...</span><img alt="loading" src="data:image/gif;base64," />
                  </>
                }
            </Form>
        </Modal.Body>
      </Modal>
      : null
  )
}